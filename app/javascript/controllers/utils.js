    ...
    return this.tokenizeText(text)
      .map((token) => {
        ...
        if (token.type === "word" && token.value.length >= 2) {
          return `<span class="word-lookup inline-block px-0.5 rounded hover:bg-blue-100 hover:text-blue-700 cursor-pointer transition-colors duration-150" data-word="${token.value}">${escapeHtml(token.value)}</span>`;
        }
