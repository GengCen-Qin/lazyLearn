...
export default class extends Controller {
  ...
  setupTextSelectionListener() {
    this.selectionBound = this.debounce(
      this.checkTextSelection.bind(this),
      100,
    );
  ...
  createFloatingIconElement() {
    ...
    // 设置样式类名
    icon.className =
      "fixed p-2 rounded-lg bg-blue-500 dark:bg-blue-600 hover:bg-blue-600 dark:hover:bg-blue-700 transition-colors duration-200 cursor-pointer shadow-lg z-40";
  ...
  async streamExplanation(text) {
    try {
      const eventSource = new EventSource(
        `/phrase_explain/stream?text=${encodeURIComponent(text)}`,
      );
  ...
  hiddenContent() {
    ...
    const examplesListEl = document.getElementById(
      "phrase-explain-examples-list",
    );
