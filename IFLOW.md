...
Lazy 是一个基于 Ruby on Rails 8.0 的视频转录和处理应用，主要功能是从小红书等平台下载视频并提供自动转录服务。该应用集成了多种转录服务（腾讯云 ASR 和 OpenAI Whisper），支持多语言转录，并提供了完整的视频管理和播放功能。应用包含用户认证系统、会话管理、文件上传（本地和腾讯云 COS）以及单词查找服务。
